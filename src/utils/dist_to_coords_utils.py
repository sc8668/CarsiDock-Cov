...
def modify_conformer(coords, values, cov_pos, aidx, edge_index, mask_rotate):
    rot_update = (torch.sigmoid(values[:3]) - 0.5) * np.pi * 2
    torsion_updates = values[3:]
    
    rot_mat = axis_angle_to_matrix(rot_update.squeeze())
    rigid_new_pos = (coords - cov_pos) @ rot_mat.T + cov_pos
    ...
    if values.shape[0] > 3:
        ...
        try:
            aligned_flexible_pos = flexible_new_pos + (cov_pos - flexible_new_pos[aidx])
