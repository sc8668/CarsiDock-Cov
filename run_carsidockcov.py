...
def main(args):
	...
	for i, mol_list in enumerate(all_mol_list):
		...
		allout_mol_list.extend(omol_list)
		if args.remove_dummyatom:
			omol_list2 = [remove_dummyat(m) for m in omol_list]
			pp1_mol_list.extend(omol_list2)	
		if args.remain_pocket:
			omol_list2 = [obtain_pose_with_pocket(pocket, m, args.covres) for m in omol_list]
			pp2_mol_list.extend(omol_list2)
		if args.remain_protein:
			omol_list2 = [obtain_pose_with_pocket(prot, m, args.covres) for m in omol_list]
			pp3_mol_list.extend(omol_list2)
	
	outdir = os.path.dirname(args.outprefix)
	if outdir != "":
		if not os.path.exists(outdir):
			os.makedirs(outdir)
	write_sdf(allout_mol_list, "%s.sdf"%args.outprefix)
	
	if args.remove_dummyatom:
		write_sdf(pp1_mol_list, "%s_nodumat.sdf"%args.outprefix)
		
	if args.remain_pocket:
		write_pdb(pp2_mol_list, "%s_withpckt"%args.outprefix, multipdb=args.multipdb)
		
	if args.remain_protein:
		write_pdb(pp3_mol_list, "%s_withprot"%args.outprefix, multipdb=args.multipdb)
